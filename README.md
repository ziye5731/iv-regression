...
For simulation, we explicitly represent the endogeneity (correlation between $\boldsymbol{x}$ and $\varepsilon_Y$) and formulate the problem as follow
...
$
\boldsymbol{\theta}_{t+1} = \boldsymbol{\theta}_t - \alpha_{t+1} ( g(\theta_{t}; \boldsymbol{x}_t) - y_t ) \nabla_{\theta} g(\boldsymbol{\theta}_t ; \boldsymbol{x}_t'),
$
